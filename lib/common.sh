...
export BLUE  # Used in other scripts
...
validate_node_id() {
    ...
    # Basic validation for node ID (alphanumeric with dashes and underscores)
...
export -f validate_json validate_wallet_address validate_node_id validate_prover_id list_node_ids edit_node_id
